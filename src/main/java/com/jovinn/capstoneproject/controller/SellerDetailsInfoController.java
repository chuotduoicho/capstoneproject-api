...
import com.jovinn.capstoneproject.dto.request.*;
...
public class SellerDetailsInfoController {
    ...
    public ResponseEntity<CertificateResponse> updateCertificate(@PathVariable(name = "id") UUID id,
                                                     @Valid @RequestBody CertificateRequest request, @CurrentUser UserPrincipal currentUser) {
        CertificateResponse response = certificateService.update(id, request, currentUser);
        return new ResponseEntity< >(response, HttpStatus.OK);
    }

    @DeleteMapping("/certificate/{id}")
    public ResponseEntity<ApiResponse> deleteCertificate(@PathVariable(name = "id") UUID id, @CurrentUser UserPrincipal currentUser) {
    ...
    public ResponseEntity<ApiResponse> deleteEducation(@PathVariable(name = "id") UUID id, @CurrentUser UserPrincipal currentUser) {
    ...
    public ResponseEntity<ApiResponse> deleteLanguage(@PathVariable(name = "id") UUID id, @CurrentUser UserPrincipal currentUser) {
        ApiResponse apiResponse = languageService.delete(id, currentUser);
        return new ResponseEntity< >(apiResponse, HttpStatus.OK);
    }

    @PostMapping("/skill")
    public ResponseEntity<SkillResponse> addEducation(@Valid @RequestBody SkillRequest request,
                                                      @CurrentUser UserPrincipal currentUser) {
    ...
    public ResponseEntity<ApiResponse> deleteSkill(@PathVariable(name = "id") UUID id, @CurrentUser UserPrincipal currentUser) {
    ...
    public ResponseEntity<ApiResponse> deleteUrlProfile(@PathVariable(name = "id") UUID id, @CurrentUser UserPrincipal currentUser) {
